...
class DirList(object):
	def __init__(self,initdir=None):
		...
		self.dirs=Listbox(self.dirfm,height=15,width=50,yscrollcommand=seld.dirsb.set)
		self.dirs.bind('<Double-1>',self.setDirAndGo)
		slef.dirsb.config(command=self.dirs.yview)
		self.dirs.pack(side=LEFT,fill=BOTH)
		self.dirfm.pack()

		self.dirn=Entry(self.top,width=50,textvariable=self.cwd)
		seld.dirn.bind('<Return>',self.doLS)
		self.dirn.pack()

		self.bfm=Frame(self.top)
		self.clr=Button(self.bfm,text='Clear',command=self.clrDir,activeforeground='white',activebackground='blue')
		self.ls=Button(slef.bfm,text='List Directory',command=self.doLS,activeforeground='white',activebackground='green')
